...
public class CuckooHash<K, V> {
	...
	public void put(K key, V value) {
		/*
		To solve this, we create a new bucket that contains the
		key value pair to be inserted. We hash the key using
		hash1 and hash2. Then, we ensure that both positions
		are empty, and that both position 1 and 2 do not already
		contain the key and value.
		Next, we loop for each iteration and check both possible
		positions. If position 1 is free, we add the new bucket
		there. If it is full, check position 2. If that is empty,
		add there. If both positions are full, we "kick out"
		the value from position 1. Next, check if position1
		is still the same hash. If it is, rehash with hash2.
		Else, rehash both position 1 and 2. If we reach max
		iterations, rehash and call put recursively to
		move the kicked out value.

		I could not get this method to work. It seems as if my recursion is
		running away. It functions for some tests but is
		kicking out on test 21 which I assume means that it is shuffling the
		buckets?
		 */
		...
		for (int i = 0; i < maxIterations; i++) {
			...
			if (table[position1] == null) {
				table[position1] = bucket;
				return;
			} else if (table[position2] == null) {
				table[position2] = bucket;
				return;
			} else {
				...
				if (position1 == hash1(bucket.getBucKey())) {
					position1 = hash2(bucket.getBucKey());
				} else {
					position1 = hash1(bucket.getBucKey());
					position2 = hash2(bucket.getBucKey());
				}
			}
		}
	...
	public V get(K key) {
		int position1 = hash1(key);
		int position2 = hash2(key);
		if (table[position1] != null && table[position1].getBucKey().equals(key))
			return table[position1].getValue();
		else if (table[position2] != null && table[position2].getBucKey().equals(key))
			return table[position2].getValue();
		return null;
	}
	...
	public boolean remove(K key, V value) {
		int position1 = hash1(key);
		int position2 = hash2(key);
		if (table[position1] != null && table[position1].getValue().equals(value)) {
			table[position1] = null;
			return true;
		}
		else if (table[position2] != null && table[position2].getValue().equals(value)) {
			table[position2] = null;
			return true;
		}
		return false;
	}
